...
###results of this are shown in VAE_models_results.py


#################
#Best model
#################
os.chdir('C:\\Users\\Jack\\Desktop\\Deep_learning\\AD_ML\\5_ML\\AD HC\\VAE_classifier')
...
model.compile(optimizer=opt, loss='categorical_crossentropy', metrics=['accuracy', metrics.AUC(name='AUC', curve ='ROC'),metrics.AUC(name='PR', curve ='PR')])
callbacks = [EarlyStopping(monitor='PR', patience=3), ModelCheckpoint(filepath='best_model.h5', monitor='PR', save_best_only=True)]
history = model.fit(X, y_data, epochs=1000, callbacks = callbacks, batch_size = 32)


###########
# Plot history of validation
###########
...
#           0      0.833     0.529     0.647       104
#           1      0.729     0.923     0.815       143
#    accuracy                          0.757       247
#   macro avg      0.781     0.726     0.731       247
#weighted avg      0.773     0.757     0.744       247
