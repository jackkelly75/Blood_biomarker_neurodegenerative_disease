...
#normlise data for MLP as it struggles with non normalised
...
#   Tune the model
#####################
#reduce tolerance as I could see it was decreasing slowly

bayes_cv_tuner = GridSearchCV(
    estimator = MLPClassifier(random_state=10, max_iter = 10000, tol = 0.0000001),
    param_grid = {
...
bayes_cv_tuner = GridSearchCV(
    estimator = MLPClassifier(random_state=10, max_iter = 5000, tol = 0.0000001),
    param_grid = {
        'activation': ['relu'],
...
result = bayes_cv_tuner.fit(X_train_VAE, y_train)
#0.6177447723829891
#{'activation': 'relu', 'alpha': 0.01, 'hidden_layer_sizes': (100,), 'solver': 'sgd'}
...
result = bayes_cv_tuner.fit(X_train_VAE, y_train)
#{'activation': 'relu', 'alpha': 0.1, 'hidden_layer_sizes': (100,), 'solver': 'sgd'}
#0.6187622308005114
...
result = bayes_cv_tuner.fit(X_train_VAE, y_train)
#0.6191421601083487
#{'activation': 'relu', 'alpha': 0.2, 'hidden_layer_sizes': (100,), 'solver': 'sgd'}

#####################
#   Run the model
#####################
clf = MLPClassifier(alpha = 0.2, activation = 'relu', hidden_layer_sizes=(100, ), solver = 'sgd', verbose=10,  random_state=10, max_iter = 10000, tol = 0.0000001)
...
#           0      0.451     0.702     0.549       104
#           1      0.635     0.378     0.474       143
#    accuracy                          0.514       247
#   macro avg      0.543     0.540     0.511       247
#weighted avg      0.558     0.514     0.505       247
